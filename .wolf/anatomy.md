...
> Auto-maintained by OpenWolf. Last scanned: 2026-07-19T21:33:51.232Z
...
## ../../../tmp/claude-1000/-home-meketreve-logica-em-jogo/8d9e190e-57d7-40a0-86df-331736df0502/scratchpad/

- `cp23-stage.mts` — Monta a cena do cp23 no servidor REAL (porta 8093, mundo plano): professor (~451 tok)
- `cp23-stage2.mts` — Tocha em pilar à frente do spawn (visível no screenshot) + halo. (~235 tok)
- `cp23-stage3.mts` — Cena completa do cp23 + platô via /regiao encher em LOTE (5390 células, (~500 tok)

## ../../../tmp/claude-1000/-home-meketreve-logica-em-jogo/96d96e4c-0809-43a9-bed0-c422babfbfe8/scratchpad/

- `nametag-fakes.mts` — Clientes falsos pro screenshot das plaquinhas de nome: 4 alunos entram e (~296 tok)
...
- `iniciar-servidor.bat` (~526 tok)
- `iniciar-servidor.sh` — ============================================================ (~465 tok)
...
- `index.html` — Lógica em Jogo (~5074 tok)
- `vite.config.ts` (~94 tok)

## client/src/

- `atlasTexture.ts` — Texture atlas procedural pintado num canvas (sem assets externos — restrição (~3526 tok)
...
- `blocksUi.ts` — Blocos colocáveis com nome em português — fonte única pra hotbar (main.ts) (~836 tok)
...
- `main.ts` — O cliente não tem filesystem: aprende os nomes das aulas pela resposta de (~13065 tok)
...
- `index.ts` — Hospedeiro Node+ws do servidor (LAN): embrulha a MESMA GameSession do Web (~3957 tok)
...
- `blocks.test.ts` (~363 tok)
- `blocks.ts` — IDs de bloco. Gravados como bytes crus nos chunks (Uint8Array), no save e no (~1727 tok)
...
- `mesher.ts` — Culled mesher: função PURA (bytes do mundo → geometria). Só emite faces que (~4336 tok)
...
- `protocol.ts` — Protocolo v0 (checkpoint 2). Mensagens JSON dos dois lados + world_snapshot (~5412 tok)
...
- `rules.ts` — Sistema GENÉRICO de atualização de bloco por vizinhança — a REGRA DE OURO (~838 tok)
- `save.test.ts` — Declares DIMS (~1454 tok)
- `save.ts` — Formato de save (.ljw) — MESMO arquivo em todos os hospedeiros: disco do (~2386 tok)
...
- `session.ts` — GameSession: o SERVIDOR autoritativo, independente de hospedeiro. (~31599 tok)
...
- `worldgen.test.ts` — Declares DIMS (~287 tok)
- `worldgen.ts` — Preset de criação de mundo (cp14): escolhido no menu/host, só vale pra (~1397 tok)
