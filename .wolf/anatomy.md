...
- `daynight.ts` — Ciclo dia/noite (cp21; astros no backlog 2026-07-19) — SÓ visual. A hora é (~4140 tok)
  - section `Keyframe` L18-77 (~761 tok)
  - fn `latticeHash` L78-86 (~116 tok)
  - fn `valueNoise` L87-106 (~226 tok)
...
- `main.ts` — O cliente não tem filesystem: aprende os nomes das aulas pela resposta de (~25606 tok)
  ...
  - fn `onSettingsChanged` L282-303 (~291 tok)
  - fn `enviarRaio` L304-339 (~486 tok)
  - fn `playerName` L340-371 (~348 tok)
  - fn `registrarChegadaDeRede` L372-379 (~60 tok)
  - fn `jitterDeRede` L380-415 (~446 tok)
  - fn `podeVoar` L416-451 (~582 tok)
  - fn `pushPanelData` L452-462 (~94 tok)
  - fn `ownDone` L463-471 (~90 tok)
  - fn `refreshObjectivesView` L472-503 (~358 tok)
  - fn `drenarFilaTroca` L504-511 (~79 tok)
  - fn `segurarAteATelaPintar` L512-521 (~109 tok)
  - fn `handleServerData` L522-665 (~1806 tok)
  - fn `connect` L666-701 (~400 tok)
...
- `meshPool.ts` — Pool de Web Workers pro mesher (2026-07-26). (~2186 tok)
  - section `ResultadoMesh` L54-69 (~138 tok)
  - class `MeshPool` L70-196 (~1298 tok)
- `meshWorker.ts` — Worker do mesher (2026-07-26). Recebe uma VIZINHANÇA (o cubo 18³ de (~453 tok)
