...
> Auto-maintained by OpenWolf. Last scanned: 2026-07-13T10:18:15.617Z
...
- `index.html` — Lógica em Jogo (~4122 tok)
...
- `blocksUi.ts` — Blocos colocáveis com nome em português — fonte única pra hotbar (main.ts) (~315 tok)
...
- `main.ts` — Checkpoint 8: menu principal. Sem parâmetro na URL o menu escolhe o rumo: (~7637 tok)
...
- `settings.ts` — Configurações do jogador, persistidas em localStorage (por navegador). (~918 tok)
...
- `index.ts` — Hospedeiro Node+ws do servidor (LAN): embrulha a MESMA GameSession do Web (~1259 tok)
...
- `session.ts` — GameSession: o SERVIDOR autoritativo, independente de hospedeiro. (~16132 tok)
