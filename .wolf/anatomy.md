...
> Auto-maintained by OpenWolf. Last scanned: 2026-07-19T23:35:20.997Z
...
- `atlasTexture.ts` — Texture atlas procedural pintado num canvas (sem assets externos — restrição (~4198 tok)
...
- `blocksUi.ts` — Blocos colocáveis com nome em português — fonte única pra hotbar (main.ts) (~952 tok)
...
- `main.ts` — O cliente não tem filesystem: aprende os nomes das aulas pela resposta de (~13890 tok)
...
- `mesher.ts` — Culled mesher: função PURA (bytes do mundo → geometria). Só emite faces que (~5732 tok)
