...
Três aulas prontas, uma por arquivo `.ljw`. Cada aula é o **mesmo mundo de cabines**:
uma cabine para o professor (no centro, onde todo mundo nasce) e uma cabine por grupo,
na fileira logo à frente.

Os `.ljw` **não vão no git** (são ~577 kB cada e são 100% regeneráveis). O que vale é o
gerador: `server/src/cenarios/gerar.ts`.

```bash
npm run cenarios                      # gera os 3 .ljw em cenarios/ (5 grupos)
